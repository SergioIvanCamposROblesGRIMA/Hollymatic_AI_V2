...
class db_connection:
    """
    Motor y sesiones de SQLAlchemy para el contenedor `app` (driver PyMySQL).

    El `app` alcanza al contenedor `db` por la red interna de Docker
    (config.DB_HOST=db, resolución por nombre de servicio; sin exponer el puerto).
    Escribe los intentos de armado (attempts / bad_assemblies).
    """
