...
class gdrive:
    """
    Subida del PDF comparativo a Google Drive y obtencion de un link
    compartible ("cualquiera con el link"). Reutiliza la misma cuenta de
    servicio que gchat.py.
    """
    ...
    def upload_and_get_link(file_path):
        ...
        remote_name = f"hollymatic_comparativa_{datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}.pdf"
        ...
        try:
            ...
            logger.info(f"PDF comparativo subido a Drive. Link: {link}")
            return link

        except HttpError as error:
            logger.critical(f"Error de la API de Drive al subir el PDF comparativo: {error}")
            return None
            ...
            logger.critical(f"Error inesperado al subir el PDF comparativo a Drive: {e}")
            return None
