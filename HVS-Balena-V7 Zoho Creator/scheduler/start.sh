...
# ─────────────────────────── constants ───────────────────────────
# Honra la variable inyectada por docker-compose/balena (p.ej. /secrets/service-account.json),
# de modo que el archivo se cree donde lo leen main.py y gchat (config.SERVICE_ACCOUNT_PATH).
readonly SERVICE_ACCOUNT_PATH="${SERVICE_ACCOUNT_PATH:-/secrets/service-account.json}"
